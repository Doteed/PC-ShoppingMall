document.addEventListener('DOMContentLoaded', () => {
    ...
    function sendFilterRequest() {
        ...
        // 서버로 POST 요청 전송
        fetch('/product/category/memoryproducts/memoryCategoryFilter', {
            method: 'POST',
	...
	function updateProductList(products) {
	        productList.innerHTML = products.map(memory => `
	            <div class="product-item">
	                <div class="product-image">
	                    <img src="/images/product/memory/memory${memory.memoryId}.png" alt="Product Image" style="width:120px; height:120px;">
	                </div>
	                <div class="product-info">
	                    <a href="/memoryproducts/${memory.memoryId}">
	                        <h2>${memory.productName}</h2>
	                    </a>
	                    <p>제조사: ${memory.manufacturer}, 제품 분류: ${memory.productClassification}, 메모리 용량: ${memory.capacity}, 동작클럭: ${memory.operatingClock}</p>
	                    <p>출시일: ${memory.releaseDate}</p>
	                </div>
	                <div class="product-price">
	                    <p>${memory.formattedPrice}</p>
	                    <button class="add-to-cart-btn" data-case-id="${memory.memoryId}">장바구니</button>
	                </div>
	            </div>
	        `).join('');
			// 장바구니 버튼 이벤트 추가
